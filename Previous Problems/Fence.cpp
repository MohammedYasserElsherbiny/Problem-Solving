...
int main()
{
    ...
    while (tt--)
    {
        ...
        vector<ll> vec(n), prefx(n + 1, 0);

        For(i, 0, n) cin >> vec[i];

        For(i, 1, n + 1) prefx[i] += prefx[i - 1] + vec[i - 1];    
        ...
        For(i, k, n + 1) 
        {
            if(mn >= prefx[i] - prefx[i - k])
            {
                ans = i;
                mn = prefx[i] - prefx[i - k];
            }
        }
...
https://codeforces.com/group/fOSxgJrX1P/contest/662586/problem/A
